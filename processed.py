...
def main():
    ...
    storage = KindOfStorage(Lock(), Value('i'))
    counter = Counter(Lock(), Value('i'))
    registry = Registry(Lock(), Manager().dict())
    ...
    parsers = [Process(target=Parser(storage, counter, registry, prefix,
